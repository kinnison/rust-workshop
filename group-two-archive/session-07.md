...
coherent modules.  At this point that other term 'cohesion' comes in a bit
...
and public associated functions, types, and other items.
...
it is defined in, and to any *child* modules of that module.  What this means
is that when you have (as is considered normal) a test submodule of your module
...
that *any* piece of code which is capable of finding a path to this item will
...
one which is public within a specified path which **must** be an ancestor of
the current module.
...
Let's consider a library crate.
...
That way, even if a user of the crate could try to *path* to something like
`throbber::CoreThingy` they wouldn't be able to actually do it because while
that item exists, the *maximal* visibility was `pub (crate)`.
...
rapidly becomes very hard to deal with.
...
the dynamic dispatch penalty.  Which is appropriate will depend on your
...
the difference between the versions of the software itself.  Semantic
versioning is split into 3 parts - major, minor, and patch.  In the general
...
minor.  If the major *and* minor are zero, then the patch is considered major.
...
For item visibility, there's a really good podcast episode of New Rustacean (ep
30) and there's also chapter 6.16 of the rust *reference*
...
tell if one version *satisfies* a version dependency limit as you might use
in a `Cargo.toml`.
